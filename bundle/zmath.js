...
/***/ function(module, exports, __webpack_require__) {
	...
	function interpolateCoons(coons) {
	  ...
	  for (let i = 1; i <= 2; i++) {
	    for (let j = 1; j <= 2; j++) {
	      b[i][j][0] = (1 - i / 3) * b[0][j][0] + i / 3 * b[3][j][0] + (1 - j / 3) * b[i][0][0] + j / 3 * b[i][3][0] - b[0][0][0] * (1 - j / 3) * (1 - i / 3) - b[3][0][0] * (j / 3) * (1 - i / 3) - b[0][3][0] * (1 - j / 3) * (i / 3) - b[3][3][0] * (j / 3) * (i / 3);

	      b[i][j][1] = (1 - i / 3) * b[0][j][1] + i / 3 * b[3][j][1] + (1 - j / 3) * b[i][0][1] + j / 3 * b[i][3][1] - b[0][0][1] * (1 - j / 3) * (1 - i / 3) - b[3][0][1] * (j / 3) * (1 - i / 3) - b[0][3][1] * (1 - j / 3) * (i / 3) - b[3][3][1] * (j / 3) * (i / 3);
	    }
	  }
	  return b;
	}
